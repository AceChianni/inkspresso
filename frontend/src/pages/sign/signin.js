...
export default function SignIn() {
  ...
  const handleSignIn = async (e) => {
    ...
    // Simulate sign-in process (e.g., API call)
  ...
  return (
    <div className="min-h-screen flex justify-center items-center">
      <form onSubmit={handleSignIn} className="bg-white p-8 rounded-md shadow-md max-w-sm w-full">
        <h2 className="text-2xl font-semibold mb-4">Sign In</h2>
        {error && <div className="text-red-500 mb-4">{error}</div>}

        <div className="mb-4">
          <label htmlFor="emailOrUsername" className="block text-gray-700 dark:text-white">Email or Username</label>
          <input
            id="emailOrUsername"
            type="text"
        ...
        <div className="mt-4 text-center">
          ...
          <Link href="/sign/signup" className="text-orange-600 hover:underline">Sign Up</Link>
        </div>
      </form>
    </div>
  );
