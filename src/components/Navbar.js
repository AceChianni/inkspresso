...
export default function Navbar({ toggleCart }) {
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 bg-base-100/80 backdrop-blur-md shadow-sm border-b border-base-300">
      ...
      {menuOpen && (
        <div className="md:hidden bg-base-100/95 backdrop-blur-md border-t border-base-300 shadow-inner px-6 py-4 space-y-4 text-center">
          ...
          <Link href="/library/checkout" onClick={() => setMenuOpen(false)} className="block nav-link">Bookshelf</Link>
          <Link href={user ? "/profile" : "/auth"} onClick={() => setMenuOpen(false)} className="block nav-link">
            {user ? "Profile" : "Sign In"}
          </Link>
        </div>
      )}
